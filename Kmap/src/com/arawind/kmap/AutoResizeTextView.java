 ...
 *  
 *  COPIED FROM http://stackoverflow.com/a/17782522
 */
...
protected void onLayout(boolean changed, int left, int top, int right, int bottom){
	...
	ArrayList<Integer> colors = (ArrayList<Integer>) this.getTag(R.string.COLORS);
...
public void setBG(int size) {
	...
	int w = getWidth()/colors.size();
	int h = getHeight();
	for(int i=0; i < colors.size(); i++){
		bg[i] = new ShapeDrawable();
		bg[i].setBounds(i*w, 0, (i+1)*w, h);			
		bg[i].getPaint().setColor(getContext().getResources().getColor(colors.get(i)));
	}
	
	bg[0].setPadding(0, getPaddingTop(), 0, getPaddingBottom());
	Drawable[] layers = bg;
